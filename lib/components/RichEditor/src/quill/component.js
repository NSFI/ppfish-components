...
var quillDirtyProps = ['modules', 'formats', 'bounds', 'theme', 'children'];
var QuillComponent = createClass({
  ...
  componentDidMount: function componentDidMount() {
    ...
    this.setState({
      ...
      if (_this.quillDelta) {
        _this.state.editor.setContents(_this.quillDelta);

        _this.state.editor.setSelection(_this.quillSelection); // this.state.editor.focus();


        _this.quillDelta = _this.quillSelection = null;
      } else if (_this.state.value) {
        _this.setEditorContents(_this.state.editor, _this.state.value);
      }
    });
  ...
  componentDidUpdate: function componentDidUpdate(prevProps, prevState) {
    var _this$state = this.state,
        ...
        isEqualContents = _this$state.isEqualContents,
  ...
  */
  cleanProps: ['value', 'id', 'className', 'style', 'placeholder', 'tabIndex', 'onChange', 'onSelectionChange', 'onFocus', 'onBlur', 'onKeyPress', 'onKeyDown', 'onKeyUp'],
  ...
  */
  renderEditingArea: function renderEditingArea() {
    ...
    var editingArea = customElement ? React.cloneElement(customElement, properties) : DOM.div(properties);
    return editingArea;
  },
  onEditorChangeText: function onEditorChangeText(value, delta, source, editor) {
    ...
    var nextContents = this.isDelta(currentContents) ? editor.getContents() : editor.getRawHTML();
  ...
  onEditorChangeSelection: function onEditorChangeSelection(nextSelection, source, editor) {
    ...
    var hasGainedFocus = !currentSelection && nextSelection; // let hasLostFocus = currentSelection && !nextSelection;
...
QuillComponent.getDerivedStateFromProps = function (nextProps, prevState) {
  ...
  if (!editor) return null; // Update only if we've been passed a new `value`.
  // This leaves components using `defaultValue` alone.

  if ('value' in nextProps) {
    var currentContents = prevState.value;
    var nextContents = nextProps.value;
    if (nextContents === prevState.lastDeltaChangeSet) throw new Error('You are passing the `delta` object from the `onChange` event back ' + 'as `value`. You most probably want `editor.getContents()` instead. ' + 'See: https://github.com/zenoamaro/react-quill#using-deltas'); // NOTE: Seeing that Quill is missing a way to prevent
    ...
    var isEqualContents = isEqual(nextContents, currentContents);
  ...
  var shouldRegenerate = some(quillDirtyProps, function (prop) {

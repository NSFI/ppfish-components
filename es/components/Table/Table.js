...
export default class Table extends React.Component {
    constructor(props) {
        ...
        this.handleColumnsFiltrateChange = (hideColumns) => {
        ...
        this.renderTable = (loading) => {
            ...
            const locale = Object.assign({}, localeDefault, this.props.locale);
            const _a = this.props, { style, className, prefixCls, showHeader, activeRowByClick } = _a, restProps = __rest(_a, ["style", "className", "prefixCls", "showHeader", "activeRowByClick"]);
            ...
            return (React.createElement(RcTable, Object.assign({ key: "table" }, restProps, { onRow: this.onRow, components: this.components, prefixCls: prefixCls, data: data, columns: columns, showHeader: showHeader, activeRowByClick: activeRowByClick, className: classString, expandIconColumnIndex: expandIconColumnIndex, expandIconAsCell: expandIconAsCell, emptyText: !loading.spinning && locale.emptyText })));
        };
    ...
    toggleTwoStateSortOrder(order, column) {
        let { sortColumn, sortOrder } = this.state;
        // 只同时允许一列进行排序，否则会导致排序顺序的逻辑问题
        let isSortColumn = this.isSortColumn(column);
        if (!isSortColumn) { // 当前列未排序
    ...
    renderColumnsFiltrate(columnsProps) {
        ...
        if (columnFiltrate) {
            const filtrateColumn = {
                key: 'filtrate-column',
                className: classNames(`${prefixCls}-filtrate-column`),
                width: 50,
                title: React.createElement(ColumnFiltrateModal, { prefixCls: prefixCls, columns: this.columns, hideColumns: this.hideColumns, defaultColumns: columnFiltrate.defaultColumns, onChange: this.handleColumnsFiltrateChange }),
            };
    ...
    renderColumnsDropdown(columns, locale) {
        ...
        return treeMap(columns, (originColumn, i) => {
            ...
            if ((column.filters && column.filters.length > 0) || column.filterDropdown) {
                ...
                filterDropdown = (React.createElement(FilterDropdown, { locale: locale, column: column, selectedKeys: colFilters, confirmFilter: this.handleFilter, prefixCls: `${prefixCls}-filter`, dropdownPrefixCls: dropdownPrefixCls || 'fishd-dropdown', getPopupContainer: this.getPopupContainer }));
            }
            if (column.sorter) {
                let isSortColumn = this.isSortColumn(column);
                if (isSortColumn) {
                ...
                if (column.sorterType === 'firstLetter') {
                    const sortButtonInner = () => {
                        if (isAscend) {
                            return (React.createElement("span", { className: `${prefixCls}-column-sorter-up ${isAscend ? 'on' : 'off'}`, onClick: () => this.toggleTwoStateSortOrder('ascend', column) },
                                React.createElement(Icon, { type: "a-z" })));
                        }
                        else if (isDescend) {
                            return (React.createElement("span", { className: `${prefixCls}-column-sorter-down ${isDescend ? 'on' : 'off'}`, onClick: () => this.toggleTwoStateSortOrder('descend', column), style: { top: -2 } },
                                React.createElement(Icon, { type: "z-a" })));
                        }
                        else {
                            return (React.createElement("span", { className: `${prefixCls}-column-sorter-up`, onClick: () => this.toggleTwoStateSortOrder('ascend', column) },
                                React.createElement(Icon, { type: "a-z" })));
                        }
                    };
    ...
    render() {
        ...
        const table = (React.createElement("div", null, this.renderTable(loading)));

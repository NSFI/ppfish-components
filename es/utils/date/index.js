...
DateUtils.i18n = {
  dayNamesShort: weeks.map(function (week) {
    return t("fishd.datepicker.weeks.".concat(week));
  }),
  dayNames: weeks.map(function (week) {
    return t("fishd.datepicker.weeks.".concat(week));
  }),
  monthNamesShort: months.map(function (month) {
    return t("fishd.datepicker.months.".concat(month));
  }),
  monthNames: months.map(function (month, index) {
    return t("fishd.datepicker.month".concat(index + 1));
  })
}; // 时间format

export var timeFormat = function timeFormat(format) {
...
export var dateFormat = function dateFormat(format) {
  if (format) return format.replace('HH:mm', '').replace(':ss', '').trim();else return 'yyyy-MM-dd';
}; // 月份数组

export var MONTH_ARRRY = ['1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月']; // 年份数组

export var YEARS_ARRAY = function YEARS_ARRAY() {
...
export var equalDateArr = function equalDateArr(arrA, arrB) {
...
export var equalYearAndMonth = function equalYearAndMonth(dateA, dateB) {
  return dateA.getFullYear() === dateB.getFullYear() && dateA.getMonth() === dateB.getMonth();
}; // 判断两个日期差(天)

export var diffDate = function diffDate(dateA, dateB) {
...
export var isValidValueArr = function isValidValueArr(value) {
...
export var setTime = function setTime(oldDate, newDate) {
...
export var isLimitRange = function isLimitRange(date, ranges) {
  var format = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'yyyy-MM-dd HH:mm:ss';

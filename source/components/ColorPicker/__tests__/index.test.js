...
describe('<ColorPicker />', () => {
  ...
  test('ColorPicker组件正常开关历史记录', () => {
    expect(wrapper.find('.fishd-color-picker-panel-history').exists()).toBe(
      true,
    );
  ...
  test('ColorPicker组件能正确设置历史记录个数', () => {
    ...
    expect(
      wrapper.find('.fishd-color-picker-panel-history-color').length,
    ).toEqual(10);
  });

  test('ColorPicker组件正常开关透明度', () => {
    expect(wrapper.find('.fishd-color-picker-panel-alpha').exists()).toBe(
      false,
    );

import { LocaleProperties } from '.';

const LocaleValue: LocaleProperties = {
  locale: 'en_US',
  AudioPlayer: {
    notSupport: 'Your browser does not support the audio tag'
  },
  AutoComplete: {
    notFoundContent: 'No data'
  },
  Cascader: {
    placeholder: 'Please select',
    notFoundContent: 'No data'
  },
  LoadMore: {
    defaultText: 'Show More',
    loadingText: 'Loading',
    errorText: 'Failed to load. Please try again',
    endText: 'No data',
  },
  Table: {
    filterTitle: 'filters',
    filterConfirm: 'OK',
    filterReset: 'Reset',
    filterEmptyText: 'No filters',
    emptyText: 'No data',
    selectAll: 'Select current page',
    selectInvert: 'Invert current page',
    selectionAll: 'Select all data',
    sortTitle: 'Sort',
    expand: 'Expand row',
    collapse: 'Collapse row',
    triggerDesc: 'Click to sort descending',
    triggerAsc: 'Click to sort ascending',
    cancelSort: 'Click to cancel sorting',
    modalTitle: 'Select the data item to be displayed',
    defaultOptionTitle: 'Default display data items',
    okText: 'OK',
    cancelText: 'Cancel',
  },
  DatePicker: {
    now: 'Now',
    today: 'Today',
    cancel: 'Cancel',
    clear: 'Clear',
    confirm: 'Confirm',
    startPlaceholder: 'Start Date',
    endPlaceholder: 'End Date',
    separator: '~',
    selectDate: 'Select Date',
    selectTime: 'Select Time',
    startDate: 'Start Date',
    startTime: 'Start Time',
    endDate: 'End Date',
    endTime: 'End Time',
    year: '',
    month1: 'Jan',
    month2: 'Feb',
    month3: 'Mar',
    month4: 'Apr',
    month5: 'May',
    month6: 'Jun',
    month7: 'Jul',
    month8: 'Aug',
    month9: 'Sep',
    month10: 'Oct',
    month11: 'Nov',
    month12: 'Dec',
    Jan: '1',
    Feb: '2',
    Mar: '3',
    Apr: '4',
    May: '5',
    Jun: '6',
    Jul: '7',
    Aug: '8',
    Sep: '9',
    Oct: '10',
    Nov: '11',
    Dec: '12',
    MONTH_ARRAY: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
    week: 'Week',
    weeks: {
      sun: 'Sun',
      mon: 'Mon',
      tue: 'Tue',
      wed: 'Wed',
      thu: 'Thu',
      fri: 'Fri',
      sat: 'Sat'
    },
    months: {
      jan: 'Jan',
      feb: 'Feb',
      mar: 'Mar',
      apr: 'Apr',
      may: 'May',
      jun: 'Jun',
      jul: 'Jul',
      aug: 'Aug',
      sep: 'Sep',
      oct: 'Oct',
      nov: 'Nov',
      dec: 'Dec'
    }
  },
  List: {
    emptyText: 'No Data'
  },
  Guide: {
    prevBtnText: 'Prev',
    nextBtnText: 'Next',
    doneBtnText: 'Got It',
    skipBtnText: 'Skip',
  },
  Modal: {
    okText: 'OK',
    cancelText: 'cancel',
  },
  Pagination: {
    items_per_page: '/ page',
    jump_to: 'Go to',
    jump_to_confirm: 'confirm',
    page: 'page',

    // Pagination.jsx
    prev_page: 'Previous Page',
    next_page: 'Next Page',
    prev_5: 'Previous 5 Pages',
    next_5: 'Next 5 Pages',
    prev_3: 'Previous 3 Pages',
    next_3: 'Next 3 Pages'
  },
   RichEditor: {
    // editor
    insertImageTip: 'JPG, JPEG, PNG, GIF, BMP format images, best display height no more than 400px, width no more than 270px.',
    rule1: '1、Single video does not exceed 10M, supports MP4, 3GP format video',
    rule2: '2、The best display height is no more than 400px',
    PleaseEnterTheVideolinkURL: "Please enter the video link URL",
    selectLocalVideo: 'Select local video',
    videoLink: 'Video link',
    localVideo: "Local video",
    insertVideo: 'Insert the video',
    selectLocalImage: "Select local image",
    insertAttachment: "Insert the attachment",
    selectLocalFile: "Select local files",
    HyperlinkAddress: 'Hyperlink address',
    insertPicture: 'Insert the picture',
    noVideoLinkErrorTip: 'Please set the video source address',
    VideoCantPlayTip: "Video doesn't play",
    file: '[file]',
    noFileInfoTip: 'Failed to read file information',
    noPicSrcTip: 'Please set the image source address',
    noVideoUrlErrorTip: 'The video link URL must not be empty',
    videoUrlFormattingError: 'Video link URL format error',
    videoLinkTooLongError: 'Video links should not exceed 1000 words',
    linkEmptyTip: 'The link address must not be empty',
    linkToolongError: 'Link addresses should not exceed 1000 words',
    noSelectionText: 'No selection text',
    insertLink: 'Insert link',
    editLink: 'Edit link',

    //toolbar
    hyperlinks: 'Hyperlink',
    backgroundColor: 'Background color',
    textDirection: 'Text direction',
    increaseIndent: 'Increase indent',
    decreaseIndent: 'Decrease indent',
    subScript: 'Sub script',
    superSciprt: 'Super sciprt',
    codeBlock: 'Code block',
    blockquote: 'Blockquote',
    strike: 'Strike',
    brushFormat: 'Brush format',
    clearFormat: 'Clear format',
    fontSize: 'Font size',
    insertEmoji: 'Insert emoji',
    defaultEmoji: 'Default emoji',
    orderedList: 'Ordered list',
    unOrderedList: 'Unordered list',
    alignJustified: 'Full-justified',
    alignCenter: 'Align center',
    alignRight: 'Align right',
    alignLeft: 'Align Left',
    fontColor: 'Font color',
    underline: 'Underline',
    italic: 'Italic',
    bold: 'Bold',
    temporarilyNoData: 'No Data',
    enterKeyWordPlease: 'Please enter the keywords',
    justOkText: 'OK',
    accessLink: 'Access the ink',
    edit: 'edit',
    deleteText: 'delete',
  },
  VideoViewer: {
    fullScreen: 'fullScreen',
    cancelFullScreen: 'cancelFullScreen',
    download: 'download',
    retry: 'retry'
  },
  Spin: {
    loading: 'Loading...',
  },
  Select: {
    errorMessage: 'Option limit exceeded',
    multipleSelectAllText: 'All select',
    notFoundContent: 'No data',
    placeholder: 'Please select',
    searchPlaceholder: 'Please enter keyword',
    selectAllText: 'Select All',
    confirmText: 'OK',
    cancelText: 'Cancel',
    loading: 'Loading...'
  },
  Transfer: {
    notFoundContent: 'No Data',
    sourceNotFoundContent: 'No Data',
    targetNotFonudContent: 'No Data',
    searchPlaceholder: 'Search here',
  },
  TreeSelect: {
    placeholder: 'Please select',
    searchPlaceholder: 'Search here',
    treeNodeResetTitle: 'Do not select any category',
    notFoundContent: 'No Data',
    okText: 'OK',
    cancelText: 'Cancel',
  },
  Upload: {
    uploading: 'Uploading',
    removeFile: 'Remove',
    uploadError: 'UploadError',
    previewFile: 'Preview',
    cancelText: 'Cancel',
    deleteText: 'DeleteAll',
    uploadedTip: '${length} item uploaded'
  },
};

export default LocaleValue;

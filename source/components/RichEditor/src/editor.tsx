...
class RichEditor extends Component<RichEditorProps, RichEditorState> {
  ...
  constructor(props: RichEditorProps) {
    ...
    this.handlers = {
      link: (value, fromAction) => {
        const { onClickToolbarBtn } = this.props;
      ...
      image: () => {
        const { onClickToolbarBtn, insertImageModalVisible } = this.props;
      ...
      attachment: () => {
        const { onClickToolbarBtn, insertAttachmentModalVisible } = this.props;
      ...
      clean: () => {
        ...
        if (range.length !== 0) {
          ...
          quill.getContents(index, length).forEach((delta, line, all) => {
            const lineLength = (delta.insert && delta.insert.length) || 0;
      ...
      customTable: value => {
        const { onClickToolbarBtn } = this.props;
        if (typeof onClickToolbarBtn == 'function' && onClickToolbarBtn('table') === false) {
          return;
        }
  ...
  handleLinkModalOk = () => {
    const el = this.linkModalInputRef.input,
      val = el.value.trim();
  ...
  handlePickLocalImage = () => {
    ...
    const handleInsertImage = info => {
      ...
      const range = this.state.curRange ? this.state.curRange : quill.getSelection(true);
  ...
  handlePickLocalFile = () => {
    const { customInsertAttachment, attachmentIconMap = {} } = this.props,
      quill = this.getEditor();
    ...
    const handleInsertFile = (file, rangeIndex) => {
      ...
      const attrs = {
        ...
        name: file.name,
        iconUrl:
          attachmentIconMap[file.type] ||
          attachmentIconMap['default'] ||
  ...
  handlePickLocalVideo = () => {
    const { customInsertVideo, videoTagAttrs } = this.props,

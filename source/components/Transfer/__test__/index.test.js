...
describe('Transfer', () => {
  ...
  it('should check all item when click on check all', () => {
    const handleSelectChange = jest.fn();
    const wrapper = mount(<Transfer {...listCommonProps} onSelectChange={handleSelectChange} />);
    wrapper.find('.fishd-transfer-list-header input[type="checkbox"]')
  ...
  it('should just check the filtered item when click on check all after search by input', () => {
    ...
    wrapper.find(TransferList).at(0).find('.fishd-transfer-list-header input[type="checkbox"]').filterWhere(n => !n.prop('checked'))
      .simulate('change');
  ...
  it('should add custom styles when their props are provided', () => {
    ...
    const wrapper = component.find('.fishd-transfer');
    const listSource = component.find('.fishd-transfer-list').first();
    const listTarget = component.find('.fishd-transfer-list').last();
    const operation = component.find('.fishd-transfer-operation').first();

...
describe('Table', () => {
  ...
  it('loading with Spin', async () => {
    ...
    expect(wrapper.find('.fishd-spin')).toHaveLength(0);
    expect(wrapper.find('.fishd-table-placeholder').text()).not.toEqual('');
    ...
    wrapper.update();
    expect(wrapper.find('.fishd-spin')).toHaveLength(1);
  });

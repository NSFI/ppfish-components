...
describe('Table.pagination', () => {
  ...
  it('should have pager when change pagination from false to undefined', () => {
    ...
    expect(wrapper.find('.fishd-pagination')).toHaveLength(1);
    expect(wrapper.find('.fishd-pagination-item-active')).toHaveLength(1);
  });
  ...
  it('change to correct page when data source changes', () => {
    ...
    expect(wrapper.find('.fishd-pagination-item-1').hasClass('fishd-pagination-item-active')).toBe(true);
  });

  it('specify the position of pagination', () => {
    const wrapper = mount(createTable({pagination: {position: 'top'}}));
    expect(wrapper.find('.fishd-spin-nested').children()).toHaveLength(2);
    expect(wrapper.find('.fishd-spin-nested').childAt(0).find('.fishd-pagination')).toHaveLength(1);
    wrapper.setProps({pagination: {position: 'bottom'}});
    expect(wrapper.find('.fishd-spin-nested').children()).toHaveLength(2);
    expect(wrapper.find('.fishd-spin-nested').childAt(1).find('.fishd-pagination')).toHaveLength(1);
    wrapper.setProps({pagination: {position: 'both'}});
    expect(wrapper.find('.fishd-spin-nested').children()).toHaveLength(3);
    expect(wrapper.find('.fishd-spin-nested').childAt(0).find('.fishd-pagination')).toHaveLength(1);
    expect(wrapper.find('.fishd-spin-nested').childAt(2).find('.fishd-pagination')).toHaveLength(1);
  });
});

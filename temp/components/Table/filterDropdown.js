...
var FilterMenu = /** @class */ (function (_super) {
    ...
    function FilterMenu(props) {
        ...
        _this.renderFilterIcon = function () {
            var _a = _this.props, column = _a.column, locale = _a.locale, prefixCls = _a.prefixCls;
    ...
    FilterMenu.prototype.hasSubMenu = function () {
        var _a = this.props.column.filters, filters = _a === void 0 ? [] : _a;
        return filters.some(function (item) { return !!(item.children && item.children.length > 0); });
    };
    FilterMenu.prototype.renderMenus = function (items) {
        ...
        return items.map(function (item) {
            if (item.children && item.children.length > 0) {
                ...
                var containSelected = Object.keys(keyPathOfSelectedItem_1).some(function (key) { return keyPathOfSelectedItem_1[key].indexOf(item.value) >= 0; });
                var subMenuCls = containSelected
                    ? _this.props.dropdownPrefixCls + "-submenu-contain-selected"
                    : '';
                return (React.createElement(SubMenu, { title: item.text, className: subMenuCls, key: item.value.toString() }, _this.renderMenus(item.children)));
            }
    ...
    FilterMenu.prototype.render = function () {
        var _a;
        var _this = this;
        var _b = this.props, column = _b.column, locale = _b.locale, prefixCls = _b.prefixCls, dropdownPrefixCls = _b.dropdownPrefixCls, getPopupContainer = _b.getPopupContainer;
        // default multiple selection in filter dropdown
        var multiple = 'filterMultiple' in column ? column.filterMultiple : true;
        var dropdownMenuClass = classNames((_a = {},
            _a[dropdownPrefixCls + "-menu-without-submenu"] = !this.hasSubMenu(),
            _a));
        var filterDropdown = column.filterDropdown;
        if (filterDropdown && typeof filterDropdown === 'function') {
            filterDropdown = filterDropdown({
                prefixCls: dropdownPrefixCls + "-custom",
                setSelectedKeys: function (selectedKeys) { return _this.setSelectedKeys({ selectedKeys: selectedKeys }); },
        ...
        var menus = filterDropdown ? (React.createElement(FilterDropdownMenuWrapper, null, filterDropdown)) : (React.createElement(FilterDropdownMenuWrapper, { className: prefixCls + "-dropdown" },
            React.createElement(Menu, { multiple: multiple, onClick: this.handleMenuItemClick, prefixCls: dropdownPrefixCls + "-menu", className: dropdownMenuClass, onSelect: this.setSelectedKeys, onDeselect: this.setSelectedKeys, selectedKeys: this.state.selectedKeys, getPopupContainer: function (triggerNode) { return triggerNode.parentNode; } }, this.renderMenus(column.filters)),
        ...
        return (React.createElement(Dropdown, { trigger: ['click'], overlay: menus, visible: this.state.neverShown ? false : this.state.visible, onVisibleChange: this.onVisibleChange, getPopupContainer: getPopupContainer, forceRender: true }, this.renderFilterIcon()));
    };

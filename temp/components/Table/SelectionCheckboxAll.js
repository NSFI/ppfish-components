...
function checkSelection(props, data, type, byDefaultChecked) {
    var store = props.store, getCheckboxPropsByItem = props.getCheckboxPropsByItem, getRecordKey = props.getRecordKey;
    // type should be 'every' | 'some'
    if (type === 'every' || type === 'some') {
        return byDefaultChecked
            ? data[type](function (item, i) { return getCheckboxPropsByItem(item, i).defaultChecked; })
            : data[type](function (item, i) { return store.getState().selectedRowKeys.indexOf(getRecordKey(item, i)) >= 0; });
    }
    return false;
}
function getCheckState(props) {
    var store = props.store, data = props.data;
    var checked;
    if (!data.length) {
        checked = false;
    }
...
var SelectionCheckboxAll = /** @class */ (function (_super) {
    ...
    SelectionCheckboxAll.prototype.render = function () {
        var _a;
        var _b = this.props, disabled = _b.disabled, prefixCls = _b.prefixCls, selections = _b.selections, getPopupContainer = _b.getPopupContainer;
        var _c = this.state, checked = _c.checked, indeterminate = _c.indeterminate;
        var selectionPrefixCls = prefixCls + "-selection";
        var customSelections = null;
        if (selections) {
            var newSelections = Array.isArray(selections)
                ? this.defaultSelections.concat(selections)
                : this.defaultSelections;
            var menu = (React.createElement(Menu, { className: selectionPrefixCls + "-menu", selectedKeys: [] }, this.renderMenus(newSelections)));
            customSelections =
                newSelections.length > 0 ? (React.createElement(Dropdown, { overlay: menu, getPopupContainer: getPopupContainer },
                    React.createElement("div", { className: selectionPrefixCls + "-down" },
                        React.createElement(Icon, { type: "down-bolder" })))) : null;
        }
        return (React.createElement("div", { className: selectionPrefixCls },
            React.createElement(Checkbox, { className: classNames((_a = {},
                    _a[selectionPrefixCls + "-select-all-custom"] = customSelections,
                    _a)), checked: checked, indeterminate: indeterminate, disabled: disabled, onChange: this.handleSelectAllChange }),
            customSelections));
    };

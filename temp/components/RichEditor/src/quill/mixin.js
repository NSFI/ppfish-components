...
var QuillMixin = {
    ...
    */
    createEditor: function ($el, config) {
        var editor = new Quill($el, config), _this = this;
        ...
        if (delta && delta.ops && Array.isArray(delta.ops)) {
            for (var i = 0; i < delta.ops.length; i++) {
                ...
                if (obj.hasOwnProperty('attributes')) {
                    var attrs = obj['attributes'] || {}, list = attrs['list'];
                    if (list)
                        return false;
                }
                // 输入内容包含非空字符时判为非空
                 ...
                 * [...insert]
                 */
                var insertChars = Array.from(insert);
                if (!insertChars.length)
                    continue;
                var notEmpty = insertChars.some(function (val) {
